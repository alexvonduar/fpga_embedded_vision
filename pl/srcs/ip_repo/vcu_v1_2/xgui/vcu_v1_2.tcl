...
proc decoder_bw_calc { DEC_VIDEO_STANDARD DEC_COLOR_DEPTH DEC_COLOR_FORMAT DEC_FRAME_SIZE ENABLE_DECODER DEC_FPS CORE_CLK NO_OF_STREAMS DEC_CODING_TYPE NO_OF_DEC_STREAMS } {
	 ...
	 set read_bw     [expr $read_bw*($NO_OF_DEC_STREAMS +1) ]
	 set write_bw    [expr $write_bw*($NO_OF_DEC_STREAMS +1) ]
	 set read_bw_a   [expr $read_bw_a*($NO_OF_DEC_STREAMS +1) ]
	 set write_bw_a  [expr $write_bw_a*($NO_OF_DEC_STREAMS +1) ]
...
proc encoder_bw_calc { ENC_CODING_TYPE ENC_VIDEO_STANDARD ENC_COLOR_DEPTH ENC_COLOR_FORMAT ENC_FRAME_SIZE ENABLE_ENCODER ENC_FPS CORE_CLK ENC_BUFFER_EN  ENC_BUFFER_B_FRAME NO_OF_STREAMS } {
    ...
    set read_bw  [ expr $read_bw*($NO_OF_STREAMS + 1) ]
    set write_bw [ expr $write_bw*($NO_OF_STREAMS + 1) ]
...
proc update_MODELPARAM_VALUE.HDL_MAX_NUM_CORES { MODELPARAM_VALUE.HDL_MAX_NUM_CORES PARAM_VALUE.NO_OF_STREAMS PARAM_VALUE.ENC_FPS PARAM_VALUE.CORE_CLK  PARAM_VALUE.ENC_FRAME_SIZE  } {
   ...
   set NO_OF_STREAMS [ expr ($NO_OF_STREAMS + 1)]
...
# kamalesh
proc validate_PARAM_VALUE.NO_OF_DEC_STREAMS { PARAM_VALUE.DEC_FRAME_SIZE PARAM_VALUE.DEC_FPS  PARAM_VALUE.NO_OF_DEC_STREAMS } {
	 ...
	 set num_cores_req [ expr ($X_VALUE*$Y_VALUE*$A_FPS*($NO_OF_DEC_STREAMS + 1)/(3840*2160*30))]
     #send_msg INFO 306 "num_cores_req $num_cores_req X_VALUE $X_VALUE Y_VALUE $Y_VALUE ENC_FPS $A_FPS NO_OF_STREAMS $NO_OF_STREAMS "
...
# added new eqn pruthvi (CR-1018128):
	 set num_cores_req [ expr ($X_VALUE*$Y_VALUE*$A_FPS*($NO_OF_STREAMS + 1)*666 + 1920*1088*60*$CORE_CLK - 1) / (1920*1088*60*$CORE_CLK)]
     #send_msg INFO 306 "num_cores_req $num_cores_req X_VALUE $X_VALUE Y_VALUE $Y_VALUE ENC_FPS $A_FPS NO_OF_STREAMS $NO_OF_STREAMS "
