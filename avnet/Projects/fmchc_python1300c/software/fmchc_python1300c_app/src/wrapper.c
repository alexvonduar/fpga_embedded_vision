...
UINT16 HAL_I2CReadBlock (UCHAR Dev, UCHAR Reg, UCHAR *Data, UINT16 NofBytes)
{
    ...
    do {
        ...
        //ret = i2c_handler->fpIicWrite(i2c_handler, i2c_addr, reg_addr, &one, 0);
        //if(ret != 0)
        //	return 0xDEAD;

        //ret = i2c_read(i2c_handler, Data + i, 1, 1);
        ret = i2c_handler->fpIicRead(i2c_handler, i2c_addr, reg_addr, Data + i, 1);
        //if(ret != 0)
        if (ret != 1)
            return 0xDEAD;
        i++;
            ...
            UINT16 NumberBytes)
{
    ...
    ret = i2c_handler->fpIicWrite(i2c_handler, i2c_addr, Reg, Data, NumberBytes);
    //if(ret != 0)
    if (ret != NumberBytes)
        goto error;
...
UCHAR HAL_I2CReadByte (UCHAR Dev, UCHAR Reg, UCHAR *Data)
{
    ...
    ret = i2c_handler->fpIicRead(i2c_handler, i2c_addr, Reg, Data, 1);
    //if(ret != 0)
    if (ret != 1)
        return 0xDD;
...
UCHAR HAL_I2CWriteByte (UCHAR Dev, UCHAR Reg, UCHAR Data)
{
    ...
    ret =  i2c_handler->fpIicWrite(i2c_handler, i2c_addr, Reg, &Data, 1);
    //if(ret != 0)
    if (ret != 1)
        return 0xDD;
