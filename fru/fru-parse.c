...
fru_get_board_tl(struct fru_common_header *header, int nr)
{
...
char* fru_alloc_get_tl(struct fru_common_header *header, int nr)
{
    struct fru_type_length *tl;
    char *res;
    int len;

    tl = fru_get_board_tl(header, nr);
    if (!tl)
        return NULL;
    len = fru_strlen(tl);
...
{
    return fru_alloc_get_tl(header, 0);
}
char *fru_get_product_name(struct fru_common_header *header)
{
    return fru_alloc_get_tl(header, 1);
}
char *fru_get_serial_number(struct fru_common_header *header)
{
    return fru_alloc_get_tl(header, 2);
}
char *fru_get_part_number(struct fru_common_header *header)
{
    return fru_alloc_get_tl(header, 3);
}
char *fru_get_fru_id(struct fru_common_header *header)
{
    return fru_alloc_get_tl(header, 4);
}
